...
For this assignment, students will be required to build a 'thing' that uses JavaScript

## Assignment description

Leveraging what lessons we learned from the JavaScript lecture today, build a small one-page simple app that accomplishes a primary goal.

**Example:** Have a page with a `<button>` that when clicked will open a Js dialog box. The user can then enter a value which will be assigned to a variable in the JavaScript.

Using the value of the variable, use functions to extract data and perform actions that are displayed to the view.
